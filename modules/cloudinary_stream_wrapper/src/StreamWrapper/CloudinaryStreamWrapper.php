...
class CloudinaryStreamWrapper implements StreamWrapperInterface {
  ...
  protected function loadResource($uri, $reset = TRUE) {
    static $resources;
    if (isset($resources[$uri]) && $resources[$uri]) {
    ...
    if (!empty($paths)) {
      ...
      $resource = $resources[$ori_uri] ?? cloudinary_stream_wrapper_load_file($public_id, [
      ...
      // It should be added width and height of original image as parameters.
      $data = cloudinary_stream_wrapper_transformation($style_name, $resource);
      if (!empty($data)) {
        $data['sign_url'] = TRUE;
        $data['type'] = $resource['type'];
        $resource['url'] = str_replace(',', '%2C', Media::fromParams($path, $data)->toUrl());

        $data['secure'] = TRUE;
        $resource['secure_url'] = str_replace(',', '%2C', Media::fromParams($path, $data)->toUrl());
      }
    ...
    elseif (!$this->resource || $reset) {
      ...
      $this->resource = cloudinary_stream_wrapper_load_file($public_id, [
        'resource_type' => CLOUDINARY_STREAM_WRAPPER_RESOURCE_IMAGE,
      ]);
  ...
  public function getExternalUrl() {
    ...
    // Check if the uri contains specific format that is used from form element.
    if (preg_match('/^(image|video|raw):(upload|private|authenticated):([^:]*)(:(.+))?$/', $target, $matches)) {
      $options['secure'] = TRUE;
      $options['resource_type'] = $matches[1];
      $options['delivery_type'] = $matches[2];
      if (isset($matches[5])) {
        $options['raw_transformation'] = $matches[5];
      }
    ...
    if (!$resource) {
      ...
      return Media::fromParams($uri)->toUrl();
    }
